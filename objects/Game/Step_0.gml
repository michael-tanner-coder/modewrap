...
// Speed settings
if (lives > 0) {room_speed = 60 * global.settings.game_speed;}

// Audio settings
audio_group_set_gain(music_group, global.settings.audio_master * global.settings.audio_music, 0);
audio_group_set_gain(sound_group, global.settings.audio_master * global.settings.audio_sounds, 0);
...
if (global.collected_coins >= goal_coin_count) {
	...
	if (global.level_index + 1 <= array_length(global.levels) - 1 && global.levels[global.level_index + 1].map != global.current_map) {
		global.paused = true;
		global.victory = true;
		return;
	}
